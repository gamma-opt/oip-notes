...
## Transportation Problem {#sec-p1l5-transportation}

This example is based on Section 7.1 of @winston2022operations.
...
These data are given in @tbl-powerco.
...
: Costs of sending 1 million kwh of electricity from plants to cities {#tbl-powerco}
...
\begin{align*}
\text{(total amount transmitted by plant 1)} &\quad \text{(plant 1 supply)} \\
...
\begin{align*}
\text{(total power transmitted to city 1)} &\quad \text{(city 1 demand)} \\
...
## Production Planning {#sec-p1l5-production}
...
@tbl-production-demand contains the projected demand from each of the 4 cities over next year, all of which must be satisfied exactly.
...
: Projected demand from the cities in terawatts {#tbl-production-demand}

@tbl-production-cost contains the expected costs of producing electricity in each of the 3 facilities, taking into account costs that change over the year.
...
: Costs of producing a terawatt of electricity (in thousands) {#tbl-production-cost}
...
\mini & f(x_{ij}, y_{ij}, s_i) = \\ 
      ...
      & + (8+50)y_{11} + (6+50)y_{12} + (10+50)y_{13} + (9+50)y_{14} + (9+50)y_{21} + (12+50)y_{22} \\
      & + (13+50)y_{23} + (7+50)y_{24} + (14+50)y_{31} + (9+50)y_{32} + (16+50)y_{33} + (5+50)y_{34} \\
...
## Combined Production-Transporation Problem {#sec-p1l5-combined}
...
Suppose the demand is the same as in @tbl-production-demand, the additional parameters are as given below, and the cost of storing electricity is €1.

|       **From \ To**       | **City 1** | **City 2** | **City 3** | **City 4** | **Battery** | **Supply  (million kwh)** |
|:---------------------:|:------:|:------:|:------:|:------:|:-------:|:---------------------:|
|        **Plant 1**        | €8     | €6     | €10    | €9     | €6      | 35                    |
|        **Plant 2**        | €9     | €12    | €13    | €7     | €4      | 50                    |
|        **Plant 3**       | €14    | €9     | €16    | €5     | €8      | 40                    |
|        **Battery**        | €4     | €2     | €7     | €5     |         |                       |
| **Demand  (million kwh)** | 45     | 20     | 30     | 30     |         |                       |

: Transportation costs for the combined problem {#tbl-combined-transportation}
...
: Production costs for the combined problem {#tbl-combined-production}
